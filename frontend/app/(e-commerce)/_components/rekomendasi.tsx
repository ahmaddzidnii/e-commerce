...
const getRecomendations = async () => {
  try {
    ...
    const data: RekomendasiProps[] = response.data.data;
    return data;
  } catch (error) {
    console.log(error);
    return [];
...
export const Rekomendasi = async () => {
  const recommendationsProducts = await getRecomendations();
  ...
  return (
    <div>
      ...
      {recommendationsProducts.length === 0 && <div className="flex items-center justify-center h-[300px]"> Internal Server Error</div>}

      <div className="grid grid-cols-12 gap-3 mt-4">
        {recommendationsProducts.map((item) => (
